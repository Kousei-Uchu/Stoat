...
const makeStructureSelectable = (structure: FolderStructure): SelectableFolderStructure => {
  ...
  if (structure.subFolders.length > 0) {
    selectableStructure.subFolders = selectableStructure.subFolders.map((folder) =>
      makeStructureSelectable(folder)
    );
  }
  return selectableStructure;
};

const removeUnselectedFolders = (selectableFolders: SelectableFolderStructure[]) => {
  ...
  for (let i = 0; i < folders.length; i += 1) {
    if (folder === undefined || folders[i].path === folder.path) {
      ...
      if (folders[i].subFolders.length > 0) {
        folders[i].subFolders = updateFolderSelectedState(folders[i].subFolders, state);
      }
        ...
        {folderComponents.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center pt-8">
            <div className="flex w-4/5 flex-col items-center justify-center text-center text-sm opacity-80">
              <span className="material-icons-round-outlined mb-4 text-5xl">folder_open</span>
        ...
        <div className="mt-10 flex items-center justify-end">
          ...
          <div className="buttons-container flex">
            ...
            <Button
              ...
              tooltipLabel={
                isAddFolderButtonDisabled ? t('addMusicFoldersPrompt.noFoldersSelected') : undefined
              }
            />
          </div>
        </div>
