...
(async () => {
  ...
  try {
    ...
    console.error('[scraper] puppeteer.launch failed:', err);
    process.exitCode = 3;
    return;
  }
  ...
  try {
    ...
    console.log('[scraper] tracklist rows visible');
    ...
    for (let i = 0; i < 60; i++) {
      ...
      console.log('[scraper] collected batch size', batch.length, 'unique', rowsMap.size);
    ...
    console.log('[scraper] finished, total tracks=', tracks.length);
    ...
    console.log('[scraper] wrote scripts/playlist_scrape_result.json');
